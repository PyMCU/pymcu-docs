...
`pymcu build` parses the Intel HEX and then **subtracts a constant 104-byte preamble** - the
interrupt vector table (102 bytes: 26 slots, the last one without its padding `nop`) plus the
